"""
This script creates a csv file containing data on EPL teams' average percentage
possession per match and their average match outcome.
"""

from bs4 import BeautifulSoup

import os
import requests
import urllib.request

# Setup [ ]

base_url = 'https://www.premierleague.com/match/'

"""
match_id = [range(5567, 5947),
			range(5947, 6327),
			range(6327, 6707),
			range(6707, 7087),
			range(7087, 7467),
			range(7467, 7847),
			range(7864, 8244),
			range(9231, 9611),
			range(9611, 9991),
			range(12115, 12495),
			range(14040, 14420),
			range(22342, 22721),
			range(38308, 38688)]

match_url = base_url + str(i)
response = requests.get(match_url)
soup = BeautifulSoup(response.text, 'html.parser')
"""

# Testing

match_url = base_url + str(38687)
response = requests.get(match_url)
soup = BeautifulSoup(response.text, 'html.parser')

# Create a new file [OK]

filename = 'epl_data.csv'

if os.path.exists(filename):
	os.remove(filename)

if not os.path.exists(filename):
	f = open(filename, 'a+')

# Append the column labels [OK]

label_str = 'name_home,pp_home,gs_home,pe_home,name_away,pp_away,gs_away,pe_away\n'
f.write(label_str)

# To do [ ]

# For each match, do the following:
	# 1) Get the 3-letter abbreviation for the home team, append to file, append a comma	[OK]
	# 2) Get the % possession value for the home team, append to file, append a comma
	# 3) Get the number of goals scored by the home team, append to file, append a comma
	# 4) Get the number of points earned by the home team, append to file, append a comma
	# 5) Get the 3-letter abbrevation for the away team, append to file, append a comma		[OK]
	# 6) Get the % possession value for the away team, append to file, append a comma
	# 7) Get the number of goals scored by the away team, append to file, append a comma
	# 8) Get the number of points earned by the away team, append to file, append a newline


# 1) Get the 3-letter abbreviation for the home team, append to file, append a comma	[OK]

name_home = soup.find('div', class_='teamScore').find_all('a')[0].contents[2][5:8];
f.write(name_home)
f.write(",")

# 2) Get the % possession value for the home team, append to file, append a comma

# Use selenium webdriver to get data from dynamically generated HTML table
# Dynamically generated table contains % possession data
# https://stackoverflow.com/questions/17597424/how-to-retrieve-the-values-of-dynamic-html-content-using-python
# https://stackoverflow.com/questions/13960326/how-can-i-parse-a-website-using-selenium-and-beautifulsoup-in-python

"""
from selenium import webdriver
from selenium.webdriver.common.keys import Keys

driver = webdriver.Chrome()
driver.get('https://www.premierleague.com/match/38687')
# assert 'premierleague' in driver.title
elem = driver.find_element_by_name('tbody')
elem.clear()
elem.send_keys('pycon')
elem.send_keys(Keys.RETURN)
assert 'No results found.' not in driver.page_source
driver.close()
"""

# Get the dynamically allocated possession figures

"""
x = soup.find('tbody', class_='matchCentreStatsContainer')
print(x)			
"""

"""
f.write(pp_home)
f.write(",")
"""

# 3) Get the number of goals scored by the home team, append to file, append a comma	[OK]

gs_home = soup.find('div', class_='score').contents[0]
f.write(gs_home)
f.write(",")

# 4) Get the number of points earned by the home team, append to file, append a comma

"""
def	calculate_points_earned(gsh, gsa):
	gs_home, gs_away = gsh, gsa
	pe_home, pe_away = 0, 0

	if (gs_home > gs_away):
		pe_home = 3
		pe_away = 0
	elif (gs_home < gs_away):
		pe_home = 0
		pe_away = 3
	else:
		pe_home, pe_away = 1

calculate_points_earned(gs_home, gs_away)
"""


# 5) Get the 3-letter abbrevation for the away team, append to file, append a comma		[OK]

name_away = soup.find('div', class_='teamScore').find_all('a')[1].contents[0][5:8];
f.write(name_away)
f.write(",")

# 6) Get the % possession value for the away team, append to file, append a comma


# 7) Get the number of goals scored by the away team, append to file, append a comma

gs_away = soup.find('div', class_='score').contents[2]
f.write(gs_away)
f.write(",")

# 8) Get the number of points earned by the away team, append to file, append a newline

f.close()
